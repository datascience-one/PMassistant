...
#### 4. Google Workspace Credentials (`credentials.json` & `token.json`)
The `sync_meetings.py` script and Scheduling agents require access to the Google Calendar API.
- **`credentials.json`**: Go to the [Google Cloud Console](https://console.cloud.google.com/). Create a new project, enable the **Google Calendar API**, and go to **Credentials**. Click **Create Credentials** -> **OAuth client ID** (select "Desktop app"). Download the JSON file and rename it to `credentials.json` in the root folder.
- **`token.json`**: This file is automatically generated the first time you run the `sync_meetings.py` or any script that requires Google auth. A browser window will open for you to log in; once you authorize the app, `token.json` will be created locally. **Do not delete this file**, as it stores your session token.

> [!IMPORTANT]
> Both `credentials.json` and `token.json` contain sensitive OAuth tokens. They have been added to `.gitignore` to prevent them from being pushed to public repositories.
